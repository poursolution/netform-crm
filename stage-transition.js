...
(function(root){
 ...
 const definitions={
  first_contact:{label:'첫 상담·요구 파악',fields:[f('needs','고객 요구사항','text',true),f('work_scope','공종·공사 범위','text',true),f('expected_timing','공사 예상시기')]},
  ...
  rapport:{label:'관계 유지',fields:[f('relationship_reason','관계관리 사유','select',true,relationshipReasons),f('relationship_reason_detail','기타 사유 직접 입력'),f('reaction','고객 반응','text',true),f('likelihood','현재 진행 가능성','select',false,['높음','보통','낮음','미확인']),f('contact_date','다음 접촉일','date',true)]},
  ...
  won:{label:'수주 · 준공 완료',fields:[f('completion_date','확인된 준공일','date',true),f('completion_checks','준공 완료 확인','multi',true,['공사 완료','준공검사 완료']),f('contract_amount','최종 수주금액(원)','money',true)]},
