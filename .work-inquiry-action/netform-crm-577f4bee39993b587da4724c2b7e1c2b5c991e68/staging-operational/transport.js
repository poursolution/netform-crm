...
(function(root){
 ...
 async function read(resource,args={}){
  ...
  if(resource==='operational'){
   ...
   if(args.after||args.deal_id||args.inquiry_id){const result=await rpc('crm_read_scoped_v2',{p_after:args.after||null,p_limit:limit,p_deal_id:args.deal_id||null,p_inquiry_id:args.inquiry_id||null});if(result?.contract_version!==2||!Array.isArray(result.deals)||!Array.isArray(result.inquiries))throw Error('READ_CONTRACT_MISMATCH');return {contract_version:1,resource,coverage:'scoped_partial',scope:'authorized_only',data:result};}
   if(!root.OperationalPagination)throw Error('READ_PAGINATION_LOAD_ORDER');
   const result=await root.OperationalPagination.collect(a=>rpc('crm_read_scoped_v2',a),{limit,maxPages:1000});
   return {contract_version:1,resource,coverage:'scoped_exhausted_per_page',scope:'authorized_only',data:result};
  }
