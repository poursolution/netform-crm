...
test('복원한 오늘업무 모듈은 기존 탭의 개편본 캐시를 우회한다',()=>{assert.match(crm,/today-work-queue\.js\?v=20260926-dedupe-3/)});
...
test('확장 종료와 보류를 제외하고 계산일을 표시한다',()=>{assert.match(queue,/!root.ExpansionFlow.converted/);assert.match(queue,/자동 계산/);assert.match(queue,/!inferred&&n!==null&&n<0/)});
