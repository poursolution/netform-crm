...
test('대시보드 문제 카드는 오늘 업무와 견적문의의 같은 운영 필터로 이어진다',()=>{assert.match(crm,/dashboardGoTodayIssue\('overdue'\)/);assert.match(crm,/dashboardGoTodayIssue\('nextMissing'\)/);assert.match(crm,/dashboardGoRisk\('noResponse'\)/);assert.match(crm,/TodayWorkQueue\.route\(kind==='nextMissing'\?'missing':kind,'pipeline'\)/);assert.match(queue,/\['stale','장기정체'\]/);assert.match(queue,/filter==='stale'\?x\.stale/);assert.match(queue,/root\.goPage\('today'\)/)});
