...
test('production shell keeps the inquiry count asset and a commit-derived build cache key', () => {
  const index = fs.readFileSync('index.html', 'utf8');
  assert.match(index, /APP_BUILD\s*=\s*'\{\{ site\.github\.build_revision \| default: "source" \}\}'/);
  assert.match(html, /inquiry-workbench\.js\?v=[^"']+/);
});
