...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>{
   ...
   window.B={deals:[deal('기한초과 현장','design','김영업',5000,'2026-09-15'),deal('오늘 현장','sent','김영업',4000,'2026-09-17'),deal('Next 없는 현장','rel','김영업',3000),deal('정체 현장','comp','김영업',2000,'2026-09-25',true),deal('계약 현장','con','김영업',1000,'2026-09-30'),deal('다른 담당 현장','design','박영업',9000,'2026-09-17'),deal('종료 현장','won','김영업',8000,'2026-09-10')]};
  ...
  assert.equal(await page.locator('.pv-section').count(),5);
  assert.equal(await page.locator('.pv-section.collapsed').count(),1);
  assert.equal(await page.locator('.pv-row').count(),6,'non-admin sees only own five open deals and one closed deal');
  assert.deepEqual(await page.locator('.pv-row').first().locator('button').allTextContents(),['응대 기록','다음 행동','상세']);
