...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>{
   ...
   window.KB5=[{k:'design',nm:'컨설팅 설계',codes:['first_contact','consulting']},{k:'sent',nm:'자료 발송완료',codes:['sent']},{k:'rel',nm:'관계 관리',sub:'(유대·침묵·대기)',codes:['rapport','silent','waiting']},{k:'comp',nm:'경쟁·임박·입찰',codes:['compete','imminent','bidding']},{k:'con',nm:'계약·시공',codes:['contract','construction','completion']},{k:'won',nm:'수주',codes:[],closed:'won'},{k:'lost',nm:'실주',codes:[],closed:'lost'}];
  ...
  assert.equal(await page.locator('.pv-section').count(),7);
  assert.deepEqual(await page.locator('.pv-title strong').allTextContents(),['컨설팅 설계','자료 발송완료','관계 관리','경쟁·임박·입찰','계약·시공','수주','실주']);
  assert.equal(await page.locator('.pv-section.collapsed').count(),0);
  assert.equal(await page.locator('.pv-row').count(),6,'non-admin sees only own five open deals and one closed deal');
  assert.equal(await page.locator('.pv-row button').count(),0,'pipeline list has no inline action buttons');
  assert.match(await page.locator('.pv-row').first().innerText(),/2026-09-15 · 2일 초과/);
