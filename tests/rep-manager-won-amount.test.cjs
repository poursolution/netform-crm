...
test('salesperson flow shows confirmed won amount instead of near-stage pipeline amount',()=>{
  assert.match(html,/repFlowDataBeforeWonAmountFix[\s\S]*r\.wonAmount=sumBy\(r\.deals\.filter[\s\S]*\),wonAmt\)/);
  ...
  assert.match(html,/head\[3\]\.textContent='수주실적'/);
});

test('salesperson coaching drawer uses the same confirmed won amount',()=>{
  assert.match(html,/repManagerOpenDrawerBeforeWonAmountFix[\s\S]*<span>확정 수주<\/span><b>'\+eok\(r\.wonAmount\)/);
});
