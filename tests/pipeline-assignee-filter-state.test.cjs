...
test('assignee chips expose filter state only, never a stale selected-site owner badge',()=>{
 ...
 assert.match(host.innerHTML,/class="rtab on"[^>]*>정정훈<b>2<\/b>/);
 assert.match(host.innerHTML,/class="rtab "[^>]*>한준엽<b>1<\/b>/);
