...
(async()=>{
 ...
 try{
  ...
  for(let i=0;i<3;i++){
   ...
   assert.equal(await page.locator('#p-brands .pipe-period').count(),1);
   assert.equal(await page.locator('#p-brands .pipe-owner').count(),1);
  ...
  console.log('PASS: 3 render cycles keep pipeline-owned controls and leave global controls untouched');
