...
test('상태 보드와 상세 목록은 같은 확장관리 데이터로 실제 렌더링된다',()=>{
 const root={innerHTML:'',querySelectorAll:()=>[]},badge={textContent:'',style:{}},rows=F.statuses.map((status,index)=>({id:'pool-'+index,sourceOpportunityId:'source-'+index,site:'검수 현장 '+index,owner:'황윤선',sourceWorkSummary:'옥상 방수',wonAmount:10000000,completionDate:'2026-03-01',nextContactAt:'2026-09-08',status,candidates:['재도장']}));
