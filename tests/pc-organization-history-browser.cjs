...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 await page.evaluate(()=>{window.calls=[];window.confirm=()=>true;window.prompt=()=> '새빛';window.toast=()=>{};window.Phase1={profile:{auth_uid:'admin-1',permission_role:'admin'},rpc:async(name,args)=>{calls.push({name,args});if(name==='crm_site_link_review_resolve_v1')return {ok:true};if(name==='crm_site_admin_search_v1')return {contract_version:1,items:[{site_id:'33333333-3333-4333-8333-333333333333',name:'새빛아파트',address:'용인시',match_reason:'이름 검색 일치',match_score:70}]};return {contract_version:2,items:[{organization_id:'11111111-1111-4111-8111-111111111111',name:'동남아파트',address:'수원시',note_count:2,status:'review_single_candidate',site_candidates:[{site_id:'22222222-2222-4222-8222-222222222222',name:'동남아파트',address:'수원시',exact_address:true,match_reason:'이름·주소 일치',match_score:150}]}]};}}});
 ...
 const payload=await page.evaluate(()=>calls.find(x=>x.name==='crm_site_link_review_resolve_v1').args);assert.deepEqual(payload,{p_organization:'11111111-1111-4111-8111-111111111111',p_resolution:'linked',p_site:'33333333-3333-4333-8333-333333333333'});
