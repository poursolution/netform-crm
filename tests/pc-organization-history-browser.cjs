...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 const selectedCalls=await page.evaluate(()=>calls.length);
 await page.evaluate(()=>{const root=document.querySelector('main');root.replaceChildren();return PCOrganizationHistory.mount(root,'동남');});
 assert.match(await page.locator('main').innerText(),/과거 예산 협의/);
 assert.equal(await page.evaluate(()=>calls.length),selectedCalls,'Parent repaint must not refetch or lose selected history');
 await page.evaluate(()=>{const root=document.querySelector('main');root.replaceChildren();return PCOrganizationHistory.mount(root,'다른');});
 assert.equal(await page.locator('[data-organization-id=org-a]').count(),0);
 assert.equal(await page.locator('[data-organization-id=org-b]').count(),1);
 assert.ok(!(await page.locator('main').innerText()).includes('과거 예산 협의'),'Changed search must discard old detail');
 await page.evaluate(()=>{hold=true;});
 await page.locator('[data-organization-id=org-b]').click();
 await page.evaluate(()=>{const root=document.querySelector('main');root.replaceChildren();PCOrganizationHistory.mount(root,'다른');pending({items:[{id:'note-pending',body:'지연 응답 메모',occurred_at:'2024-01-01'}],has_more:false});hold=false;});
 await page.getByText('지연 응답 메모',{exact:true}).waitFor();
 await page.evaluate(()=>dispatchEvent(new Event('phase1:profile')));
 assert.equal(await page.locator('[data-organization-history]').count(),0,'Same-identity profile refresh must clear old-authority content');
 await page.evaluate(()=>PCOrganizationHistory.mount(document.querySelector('main'),'동남'));
 ...
 console.log('PASS: PC integration, repaint selection preserved without refetch, changed search clears detail, profile/identity clearing, missing API error. Mock integration.');
