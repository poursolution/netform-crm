...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 await page.evaluate(()=>{window.calls=[];window.confirm=()=>true;window.toast=()=>{};window.Phase1={profile:{auth_uid:'admin-1',permission_role:'admin'},rpc:async(name,args)=>{calls.push({name,args});if(name==='crm_site_link_review_resolve_v1')return {ok:true};return {contract_version:1,items:[{organization_id:'11111111-1111-4111-8111-111111111111',name:'동남아파트',address:'수원시',note_count:2,status:'review_single_candidate',site_candidates:[{site_id:'22222222-2222-4222-8222-222222222222',name:'동남아파트',address:'수원시',exact_address:true}]}]};}}});
 ...
 await page.getByText('동남아파트',{exact:true}).waitFor();assert.match(await page.locator('main').innerText(),/주소 일치/);
