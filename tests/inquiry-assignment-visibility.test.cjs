...
const profiles={
  '황윤선':{active:true,salesRep:true,role:'sales'},
  '조재연':{active:true,salesRep:false,role:'consultation'},
  '한인규':{active:false,salesRep:false,role:'inactive'},
  '경남지사':{active:true,salesRep:false,role:'branch_pool'}
};
const context={
  repN:value=>String(value||'').trim()||'미배정',
  repProfile:name=>profiles[name]||{active:true,salesRep:false,role:'unknown'}
};
