...
assert.equal(remaining.length,4,'Only general deal amount, expansion normalization and two stage-write paths remain');
assert.ok(remaining.every(l=>/function dealAmt|function normalizeExpansion|target===/.test(l)));
let scripts=0;
