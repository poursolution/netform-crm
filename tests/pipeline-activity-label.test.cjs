...
function render(activities){
 const ctx={itemPatch:()=>({}),priorityInfo:()=>({}),actionObj:()=>null,contactInfo:()=>({}),dealKey:()=> 'test',repN:()=>'',stageNoLabel:()=>'',dealStage:()=>'',constructionYearOf:()=> '미입력',fmtAmt:()=>'',oppAmt:()=>0,quoteAmt:()=>0,splitInlinePanel:()=>'',esc:x=>String(x??'').replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;')};
 ...
 return ctx.quickPanelHTML({site:'합성 현장',activities});
}
