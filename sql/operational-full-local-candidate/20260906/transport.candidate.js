...
(function(root){
 'use strict';
 const REF='rprechiaglyjaydkmxsu',VERSION=2,c=root.PHASE1_CONFIG;
 ...
 async function read(resource,args={}){
  ...
  if(resource==='operational'){
   ...
   async function collect(domain){const items=[],seen=new Set();let after=null;for(let pageNo=0;pageNo<500;pageNo++){const result=await rpc('crm_operational_source_v1',{p_domain:domain,p_after:after,p_limit:limit}),p=result?.pagination;if(result?.contract_version!==1||result.resource!=='operational_source'||result.domain!==domain||result.scope_completeness!=='actor_authorized_rows_only'||!Array.isArray(result.items)||!p||!['complete','partial'].includes(p.completeness)||typeof p.has_more!=='boolean'||p.has_more!==(p.completeness==='partial')||p.has_more!==(typeof p.next_cursor==='string'))throw Error('READ_CONTRACT_MISMATCH');for(const item of result.items){if(!item?.id||seen.has(item.id))throw Error('READ_CONTRACT_MISMATCH');seen.add(item.id);items.push(item);}if(!p.has_more)return items;if(p.next_cursor===after)throw Error('READ_CURSOR_STALLED');after=p.next_cursor;}throw Error('READ_INCOMPLETE');}
   const [deals,inquiries,expansion_pool,customer_support_actions,message_logs,user_directory,rep_manager_comments]=await Promise.all([collect('deal_core'),collect('inquiry_core'),collect('expansion_pool'),collect('customer_support_action'),collect('message_log'),collect('user_directory'),collect('rep_manager_comment')]);
   return {contract_version:1,resource,coverage:'complete_for_actor_scope',scope:'actor_authorized_rows_only',data:{contract_version:6,deals,inquiries,expansion_pool,customer_support_actions,customerSupportActions:customer_support_actions,message_logs,messageLogs:message_logs,user_directory,rep_manager_comments,expansion_events:expansion_pool.flatMap(x=>Array.isArray(x.events)?x.events:[])}};
  }
 ...
 root.Phase1=Object.freeze({config:c,createClient,admit,beginLogin,signOut,storage,mode,read,rpc,queue:{enqueue,flush,list,validateAck},uploadAttachment,get profile(){return profile;},requests,blocked,
