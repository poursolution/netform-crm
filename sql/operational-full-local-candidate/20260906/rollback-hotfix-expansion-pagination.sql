...
DO $guard$ BEGIN
 ...
 THEN RAISE EXCEPTION 'expansion pagination hotfix rollback drift'; END IF;
END $guard$;
...
BEGIN
 IF p_domain='expansion_pool' THEN
  ...
  SELECT coalesce(jsonb_agg(item ORDER BY (item->>'id')::uuid) FILTER(WHERE rn<=p_limit),'[]'::jsonb),max(rn)>p_limit,(max(item->>'id') FILTER(WHERE rn<=p_limit))::uuid
