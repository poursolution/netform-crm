...
BEGIN
 ...
 old_block:='INSERT INTO public.contact_assignments(person_key,opportunity_id,site_name,office_phone,started_at,status,reason)'||chr(10)||
 ...
 new_block:='UPDATE public.contact_assignments ca SET office_phone=nullif(p_payload->>''to_office_phone'',''''),started_at=(p_payload->>''moved_at'')::date,status=''current'',reason=reason'||chr(10)||
  ...
  '    END IF;';
 IF position(old_block in ddl)=0 THEN RAISE EXCEPTION 'contact move upsert drift'; END IF;
