-- Conservative, review-only candidate ranking for orphan legacy organizations.
...
begin
 ...
 with pending as (
  select o.id organization_id,o.name,o.address,(select count(*) from public.notes n where n.organization_id=o.id and crm_security.can_read_legacy_note(n.deal_id,n.organization_id)) note_count,
  ...
  select p.organization_id,p.name,p.address,p.note_count,coalesce((select jsonb_agg(jsonb_build_object(
