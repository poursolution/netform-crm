...
function consultingOwnerBoard(scopedOwner){
 ...
 return '<details class="sw-owner-fold"'+(scopedOwner!=='전체'?' open':'')+'><summary>담당자별 현황 <small>펼쳐서 담당자별로 좁혀 보기</small></summary><section class="sw-owner-board" aria-label="담당자별 컨설팅 현황"><header><h3>담당자별 현황</h3><small>위험 높은 순 · 행 클릭 = 아래 목록 필터</small>'+btn('전체 보기','owner','전체')+'</header><div class="sw-table-scroll"><table><thead><tr>'+['담당자','담당','합계 금액','초과','Next 없음','정보 보완','최장 초과',''].map(t=>'<th scope="col">'+h(t)+'</th>').join('')+'</tr></thead><tbody>'+body+'</tbody></table></div></section></details>';
}
...
function competitionOwnerBoard(scopedOwner){
 ...
 return '<details class="sw-owner-fold"'+(scopedOwner!=='전체'?' open':'')+'><summary>담당자별 현황 <small>펼쳐서 담당자별로 좁혀 보기</small></summary><section class="sw-owner-board" aria-label="담당자별 경쟁·임박·입찰 현황"><header><h3>담당자별 현황</h3><small>위험 높은 순 · 행 클릭 = 아래 목록 필터</small>'+btn('전체 보기','owner','전체')+'</header><div class="sw-table-scroll"><table><thead><tr>'+['담당자','담당','합계 금액','지난 결정','D-3 이내','일정 미등록','최장 경과',''].map(t=>'<th scope="col">'+h(t)+'</th>').join('')+'</tr></thead><tbody>'+body+'</tbody></table></div></section></details>';
}
...
function constructionFrame(items,list){
 ...
 shown.forEach(x=>{
  ...
  const startChip=v.startDate?(sd!=null&&sd<0?'<span class="sw-due-chip mut">착공 '+h(String(v.startDate).slice(5,10))+'</span>':sd===0?'<span class="sw-due-chip warn">오늘 착공</span>':'<span class="sw-due-chip ok">착공 D-'+sd+'</span>'):'<span class="sw-due-chip mut">-</span>';
