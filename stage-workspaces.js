...
const due=r=>r.days==null?'일정 미지정':r.days<0?Math.abs(r.days)+'일 지남':r.days===0?'오늘':r.days+'일 남음';
...
function card(r,body,label='처리',action='process'){return '<article class="sw-card" data-deal="'+a(r.key)+'"><header><div>'+btn(r.site,'record',r.key)+(root.advisoryBadge?root.advisoryBadge(r.item):'')+'<small>'+h(r.owner||'미배정')+' · '+h(root.dealWorkSummary(r.item)||'공종 미기록')+'</small></div></header>'+body+'<div class="sw-action">'+btn(label,action,r.key)+'</div></article>';}
...
function relationshipOwnerBoard(scopedOwner){
 ...
 return '<details class="sw-owner-fold"'+(scopedOwner!=='전체'?' open':'')+'><summary>담당자별 현황 <small>펼쳐서 담당자별로 좁혀 보기</small></summary><section class="sw-owner-board" aria-label="담당자별 관계관리 현황"><header><h3>담당자별 현황</h3><small>위험 높은 순 · 행 클릭 = 아래 목록 필터</small>'+btn('전체 보기','owner','전체')+'</header><div class="sw-table-scroll"><table><thead><tr>'+['담당자','유대','침묵','대기','합계 금액','연락 초과','침묵 전환','발송 대기','최장 연락 공백',''].map(t=>'<th scope="col">'+h(t)+'</th>').join('')+'</tr></thead><tbody>'+body+'</tbody></table></div></section></details>';
}
function relationship(list){
 ...
 shown.forEach(x=>{
  ...
  const nextChip=r.code==='waiting'?(x.resume?'<span class="sw-due-chip mut">재개 '+h(String(x.resume).slice(0,10))+'</span>':'<span class="sw-due-chip warn">재개일 없음</span>'):(r.days==null?'<span class="sw-due-chip warn">연락일 없음</span>':r.days<0?'<span class="sw-due-chip hot">'+Math.abs(r.days)+'일 지남</span>':r.days===0?'<span class="sw-due-chip warn">오늘</span>':'<span class="sw-due-chip ok">'+r.days+'일 남음</span>');
...
function consultingOwnerBoard(scopedOwner){
 ...
 return '<details class="sw-owner-fold"'+(scopedOwner!=='전체'?' open':'')+'><summary>담당자별 현황 <small>펼쳐서 담당자별로 좁혀 보기</small></summary><section class="sw-owner-board" aria-label="담당자별 컨설팅 현황"><header><h3>담당자별 현황</h3><small>위험 높은 순 · 행 클릭 = 아래 목록 필터</small>'+btn('전체 보기','owner','전체')+'</header><div class="sw-table-scroll"><table><thead><tr>'+['담당자','담당','합계 금액','초과','다음 할 일 없음','정보 보완','최장 초과',''].map(t=>'<th scope="col">'+h(t)+'</th>').join('')+'</tr></thead><tbody>'+body+'</tbody></table></div></section></details>';
}
...
function sentOwnerBoard(scopedOwner){
 ...
 return '<details class="sw-owner-fold"'+(scopedOwner!=='전체'?' open':'')+'><summary>담당자별 현황 <small>펼쳐서 담당자별로 좁혀 보기</small></summary><section class="sw-owner-board" aria-label="담당자별 발송 후속 현황"><header><h3>담당자별 현황</h3><small>위험 높은 순 · 행 클릭 = 아래 목록 필터</small>'+btn('전체 보기','owner','전체')+'</header><div class="sw-table-scroll"><table><thead><tr>'+['담당자','담당','합계 금액','후속 초과','오늘 확인','후속일 미지정','최장 초과',''].map(t=>'<th scope="col">'+h(t)+'</th>').join('')+'</tr></thead><tbody>'+body+'</tbody></table></div></section></details>';
}
function sentFrame(items){
 ...
 shown.forEach(x=>{
  ...
  const chip=days==null?'<span class="sw-due-chip warn">미지정</span>':days<0?'<span class="sw-due-chip hot">'+Math.abs(days)+'일 지남</span>':days===0?'<span class="sw-due-chip warn">오늘</span>':'<span class="sw-due-chip ok">'+days+'일 남음</span>';
...
function consultingFrame(items){
 ...
 shown.forEach(x=>{
  ...
  const chip=qd==null?'<span class="sw-due-chip mut">미지정</span>':qd<0?'<span class="sw-due-chip hot">'+Math.abs(qd)+'일 지남</span>':qd===0?'<span class="sw-due-chip warn">오늘</span>':'<span class="sw-due-chip ok">'+qd+'일 남음</span>';
...
function competitionOwnerBoard(scopedOwner){
 ...
 return '<details class="sw-owner-fold"'+(scopedOwner!=='전체'?' open':'')+'><summary>담당자별 현황 <small>펼쳐서 담당자별로 좁혀 보기</small></summary><section class="sw-owner-board" aria-label="담당자별 경쟁·임박·입찰 현황"><header><h3>담당자별 현황</h3><small>위험 높은 순 · 행 클릭 = 아래 목록 필터</small>'+btn('전체 보기','owner','전체')+'</header><div class="sw-table-scroll"><table><thead><tr>'+['담당자','담당','합계 금액','지난 결정','3일 안','일정 미등록','최장 경과',''].map(t=>'<th scope="col">'+h(t)+'</th>').join('')+'</tr></thead><tbody>'+body+'</tbody></table></div></section></details>';
}
function schedule(items){
 ...
 shown.forEach(x=>{
  ...
  if(isFirst&&!markedFirst){markedFirst=true;rowsHtml+='<tr class="sw-group-row hot"><td colspan="7">먼저 볼 것 — 지난 결정 · 오늘 · 내일 · 일정 미등록 ('+first.length+')</td></tr>';}
  if(!isFirst&&!markedRest){markedRest=true;rowsHtml+='<tr class="sw-group-row"><td colspan="7">예정된 결정 ('+rest.length+')</td></tr>';}
  const chip=n==null?'<span class="sw-due-chip warn">미등록</span>':n<0?'<span class="sw-due-chip hot">'+Math.abs(n)+'일 지남</span>':n===0?'<span class="sw-due-chip warn">오늘</span>':'<span class="sw-due-chip '+(n<=3?'warn':'ok')+'">'+n+'일 남음</span>';
...
/* 계약·시공 v2(A안): 담당자 현황판 + 계약실적 원장 + 진행 세그먼트 + 표. */
const CONS_STAGES={contract:'계약',construction:'시공',completion:'준공'};
function constructionOwnerBoard(scopedOwner){
 const src=(root.PipelineWorkspace&&root.PipelineWorkspace.rows?root.PipelineWorkspace.rows({unscoped:true}):[]).filter(r=>r.group==='construction');
 const ledger=root.ContractSalesData?root.ContractSalesData.state():null,contracts=new Map(((ledger&&ledger.items)||[]).map(c=>[String(c.deal_id),c]));
 const map=new Map();
 src.forEach(r=>{
  const o=r.owner||'미배정',row=map.get(o)||{owner:o,count:0,amount:0,pending:0,building:0,done:0};
  row.count++;
  const c=contracts.get(String(r.item.id)),cf=r.item.stage_contexts?.contract?.fields||{};
  row.amount+=Number((c&&c.balance)??cf.contract_amount??r.fields.contract_amount)||0;
 ...
 const total=rows.reduce((s,x)=>({count:s.count+x.count,amount:s.amount+x.amount,pending:s.pending+x.pending,building:s.building+x.building,done:s.done+x.done}),{count:0,amount:0,pending:0,building:0,done:0});
 const body=rows.map(x=>'<tr data-ps-action="owner" data-value="'+a(x.owner)+'"'+(scopedOwner===x.owner?' class="sel"':'')+'><td><span class="sw-light '+x.light+'"></span><b>'+h(x.owner)+'</b></td><td>'+x.count+'</td><td class="sw-amt">'+h(money(x.amount).replace('금액 미입력','-'))+'</td><td class="'+(x.pending?'sw-bad':'sw-zero')+'">'+x.pending+'</td><td class="'+(x.building?'sw-warn':'sw-zero')+'">'+x.building+'</td><td class="'+(x.done?'sw-warn':'sw-zero')+'">'+x.done+'</td><td class="sw-go">'+(scopedOwner===x.owner?'전체 보기':'목록 보기')+'</td></tr>').join('')
  +'<tr class="sw-total"><td>전체</td><td>'+total.count+'</td><td class="sw-amt">'+h(money(total.amount).replace('금액 미입력','-'))+'</td><td class="'+(total.pending?'sw-bad':'sw-zero')+'">'+total.pending+'</td><td class="'+(total.building?'sw-warn':'sw-zero')+'">'+total.building+'</td><td class="'+(total.done?'sw-warn':'sw-zero')+'">'+total.done+'</td><td></td></tr>';
 return '<details class="sw-owner-fold"'+(scopedOwner!=='전체'?' open':'')+'><summary>담당자별 현황 <small>펼쳐서 담당자별로 좁혀 보기</small></summary><section class="sw-owner-board" aria-label="담당자별 계약·시공 현황"><header><h3>담당자별 현황</h3><small>계약대기 많은 순 · 행 클릭 = 아래 목록 필터</small>'+btn('전체 보기','owner','전체')+'</header><div class="sw-table-scroll"><table><thead><tr>'+['담당자','담당','계약금액 합','계약대기','시공중','준공확인',''].map(t=>'<th scope="col">'+h(t)+'</th>').join('')+'</tr></thead><tbody>'+body+'</tbody></table></div></section></details>';
}
function constructionFrame(items,list){
 const f=root.G.pipelineQueue;
 const scopedOwner=root.SalesScope&&root.SalesScope.state?root.SalesScope.state().owner:'전체';
 const seg=root.G.consSeg&&CONS_STAGES[root.G.consSeg]?root.G.consSeg:'all';
 const segFiltered=items.filter(x=>seg==='all'||x.row.code===seg);
 const first=segFiltered.filter(x=>x.priority===0).slice().sort((p,q)=>String(p.values.contractDate||'9999').localeCompare(String(q.values.contractDate||'9999')));
 const rest=segFiltered.filter(x=>x.priority!==0).slice().sort((p,q)=>p.priority-q.priority||String(p.values.startDate||'9999').localeCompare(String(q.values.startDate||'9999')));
 const ordered=first.concat(rest);
 const segBar='<nav class="sw-seg" aria-label="계약·시공 구분">'+[['all','전체',items.length],['contract','계약',items.filter(x=>x.row.code==='contract').length],['construction','시공',items.filter(x=>x.row.code==='construction').length],['completion','준공',items.filter(x=>x.row.code==='completion').length]].map(([k,label,n])=>'<button type="button" data-ps-action="consseg" data-value="'+k+'" aria-pressed="'+(seg===k)+'">'+h(label)+' <b>'+n+'</b></button>').join('')+'</nav>';
 const pages=Math.max(1,Math.ceil(ordered.length/30));f.page=Math.min(f.page||1,pages);
 const shown=ordered.slice((f.page-1)*30,f.page*30);
 let markedFirst=false,markedRest=false,rowsHtml='';
 shown.forEach(x=>{
...
function wonOwnerBoard(scopedOwner){
 ...
 return '<details class="sw-owner-fold"'+(scopedOwner!=='전체'?' open':'')+'><summary>담당자별 수주 실적 <small>펼쳐서 담당자별로 좁혀 보기</small></summary><section class="sw-owner-board" aria-label="담당자별 수주 실적"><header><h3>담당자별 수주 실적</h3><small>수주 금액 순 · 행 클릭 = 아래 목록 필터</small>'+btn('전체 보기','owner','전체')+'</header><div class="sw-table-scroll"><table><thead><tr>'+['담당자','수주','수주 금액','금액 미확인','준공 완료',''].map(t=>'<th scope="col">'+h(t)+'</th>').join('')+'</tr></thead><tbody>'+body+'</tbody></table></div></section></details>';
}
...
function lostOwnerBoard(scopedOwner){
 ...
 return '<details class="sw-owner-fold"'+(scopedOwner!=='전체'?' open':'')+'><summary>담당자별 실주 현황 <small>펼쳐서 담당자별로 좁혀 보기</small></summary><section class="sw-owner-board" aria-label="담당자별 실주 현황"><header><h3>담당자별 실주 현황</h3><small>사유 미기록 많은 순 · 행 클릭 = 아래 목록 필터</small>'+btn('전체 보기','owner','전체')+'</header><div class="sw-table-scroll"><table><thead><tr>'+['담당자','실주','실주 금액','사유 미기록','재접촉 검토',''].map(t=>'<th scope="col">'+h(t)+'</th>').join('')+'</tr></thead><tbody>'+body+'</tbody></table></div></section></details>';
}
