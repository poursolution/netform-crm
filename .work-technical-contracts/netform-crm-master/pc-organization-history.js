...
(function(w){
 ...
 function mount(root,query=''){
  ...
  section=node('section',undefined,'site-master-section site-link-review');section.dataset.organizationHistory='true';section.hidden=true;let items=[],filter='all',page=1;
  ...
  const summary=node('p','과거자료를 기존 현장에 연결하거나 별도 현장으로 확정합니다. 이름은 후보 검색에만 사용됩니다.');titles.append(node('h4','과거자료 연결 검토'),summary);
  const retry=node('button','다시 조회','btn'),close=node('button','목록 닫기','secondary');retry.type=close.type='button';close.onclick=()=>{section.hidden=true;};const headActions=node('div');headActions.append(retry,close);head.append(titles,headActions);section.append(head,toolbar,list,pager);root.append(section);
  ...
  function render(){renderToolbar();const filtered=items.filter(x=>filter==='all'||filter==='exact'&&hasExact(x)||filter==='name'&&hasName(x)||filter==='separate'&&!(x.site_candidates||[]).length),pages=Math.max(1,Math.ceil(filtered.length/PAGE_SIZE));page=Math.min(page,pages);const rows=filtered.slice((page-1)*PAGE_SIZE,page*PAGE_SIZE);if(!rows.length){message(list,search?'검색·필터 조건에 맞는 연결 검토 건이 없습니다.':'연결 확인이 필요한 과거자료가 없습니다.');pager.replaceChildren();return;}const frag=document.createDocumentFragment();
   rows.forEach(row=>{
    ...
    const select=node('select');select.setAttribute('aria-label',(row.name||'과거 고객')+' 연결할 현장');select.append(node('option','연결할 현장 선택'));
    (row.site_candidates||[]).forEach(s=>{const o=node('option',candidateLabel(s));o.value=s.site_id;select.append(o)});
    const link=node('button','선택 현장에 연결'),searchButton=node('button','다른 현장 찾기','secondary'),separate=node('button','별도 현장','secondary');link.type=searchButton.type=separate.type='button';link.disabled=true;
    async function resolve(resolution,site){const assets='과거 메모 '+Number(row.note_count||0)+'건 · 담당자 '+contacts+'명',label=resolution==='linked'?'선택한 현장에 '+assets+'을 연결':'새 현장을 만들고 '+assets+'을 연결';if(!w.confirm(label+'하시겠습니까?'))return;link.disabled=separate.disabled=true;try{const ack=await transport.rpc('crm_site_link_review_resolve_v1',{p_organization:row.organization_id,p_resolution:resolution,p_site:site||null});if(!ackMatches(ack,row,resolution))throw Error('ACK_MISMATCH');assetActor='';w.toast(resolution==='separate'?'새 현장을 생성하고 과거자료를 연결했습니다.':'과거자료 연결을 확정했습니다.');await refresh();if(typeof w.paintSites==='function')w.paintSites();}catch(e){link.disabled=!select.value;separate.disabled=false;w.toast('연결 결과를 확인하지 못했습니다. 다시 시도해 주세요.');}}
  ...
  async function refresh(){
   ...
   }catch(e){if(ticket===generation)message(list,'과거자료 연결 검토 목록을 불러오지 못했습니다.',true);}
  }
