/* PC-only derived transport. Keep aligned with transport.js; explicit RPC extensions only. */
(function(root){
 'use strict';
 ...
 const rpcAllow=new Set(['crm_advisory_library_read_v1','crm_advisory_site_read_v1','crm_advisory_ledger_pending_v1','crm_advisory_bid_summary_v1','crm_advisory_attribution_v1','crm_advisory_attribution_decide_v1','crm_contract_sales_read_v1','crm_contract_sales_write_v1','crm_inquiry_manual_capability_v1','crm_inquiry_manual_create_v1','crm_site_address_update_v1','crm_site_identity_relink_v1','crm_site_record_relink_v1','crm_site_record_corrected_separate_v1','crm_site_admin_search_v1','crm_site_linked_history_v1','crm_site_linked_assets_v1','crm_site_link_review_list_v1','crm_site_link_review_resolve_v1','crm_site_record_link_review_list_v1','crm_site_record_link_review_resolve_v1','crm_orphan_organization_history','crm_manager_request_create_v1','crm_manager_request_list_v1','crm_profile_scoped_v2','crm_read_scoped_v2','crm_contacts_scoped_v2','crm_write_command_v2','crm_operational_source_v1','crm_operational_changes_v1','crm_expansion_note','crm_expansion_context','crm_admin_list_accounts_v1','crm_admin_reset_password_v1','crm_release_manifest_v1']);
 ...
 root.fetch=async function(input,init){const u=allowed(input);requests.push({origin:u.origin,path:u.pathname,method:init?.method||'GET'});const res=await fetchNative(input,init);
  ...
  if(res&&res.status===404&&u.origin===origin&&u.pathname.startsWith('/rest/v1/rpc/')){const fn=u.pathname.slice(13);res.clone().json().then(b=>{if(b&&b.code==='PGRST202')root.CRMRelease?.noteMissing(fn);}).catch(()=>{});}
  return res;};
 ...
 /* 상세 새 창(2026-09-26 대표 "상세보기 그냥 새창으로"): 이 CRM이 연 상세 창(?solo=1)은 여는 창의 로그인 보관소를 같이 쓴다.
    창마다 따로 토큰을 갱신하면 Supabase가 '갱신 토큰 재사용'으로 보고 두 창 모두 로그아웃시킨다 — 읽을 때 여는 창의 최신본을 가져오고,
    갱신하면 여는 창에도 적는다(갱신 순서는 로그인 라이브러리의 창 간 잠금이 맞춘다). 여는 창이 닫히면 자기 사본으로 이어 간다. */
 const soloAuth=(()=>{try{return new URL(root.location.href).searchParams.get('solo')==='1'&&!!root.opener&&root.opener.location.origin===root.location.origin;}catch{return false;}})();
 function sharedAuth(){if(!soloAuth)return null;try{return root.opener&&!root.opener.closed?root.opener.sessionStorage:null;}catch{return null;}}
 const authStorage={getItem(){const shared=sharedAuth();if(shared){try{const u=shared.getItem(base+'auth-locator'),r=u&&shared.getItem(base+u+':auth');if(u&&r){nativeSession.setItem(base+'auth-locator',u);nativeSession.setItem(base+u+':auth',r);}}catch{}}const uid=nativeSession.getItem(base+'auth-locator');if(!uid)return null;const raw=nativeSession.getItem(base+uid+':auth');try{const s=JSON.parse(raw);if(!s||s.user?.id!==uid||typeof s.access_token!=='string'||typeof s.refresh_token!=='string')throw Error('INVALID_SESSION');return raw;}catch{invalidate();return null;}},
  setItem(key,value){const session=JSON.parse(value),uid=session.user?.id;if(!uid)throw Error('INVALID_AUTH_STORAGE');const old=nativeSession.getItem(base+'auth-locator');if(old&&old!==uid){invalidate();channel?.postMessage('identity-change');}nativeSession.setItem(base+'auth-locator',uid);nativeSession.setItem(base+uid+':auth',value);const shared=sharedAuth();if(shared){try{shared.setItem(base+'auth-locator',uid);shared.setItem(base+uid+':auth',value);}catch{}}},
 ...
 async function read(resource,args={}){
  ...
  if(resource==='operational'){
   ...
   const knownDomains=['deal_core','inquiry_core','expansion_pool','customer_support_action','message_log','campaign_core','asq_project'],domains=args.domains===undefined?knownDomains:args.domains;
  ...
  if(resource==='operational_changes'){const since=args.since==null?null:String(args.since);if(since!==null&&!Number.isFinite(Date.parse(since)))throw Error('INVALID_SINCE');const result=await rpc('crm_operational_changes_v1',{p_since:since,p_limit:500}),ids=v=>Array.isArray(v)&&v.every(x=>x&&UUID_RE.test(String(x.id||''))&&typeof x.v==='boolean');if(result?.ok!==true||result.contract_version!==1||typeof result.server_time!=='string'||!(result.expired===true||ids(result.deals)&&ids(result.inquiries)))throw Error('READ_CONTRACT_MISMATCH');return {contract_version:1,resource,coverage:'complete',scope:'changed_ids_with_visibility',data:result};}
