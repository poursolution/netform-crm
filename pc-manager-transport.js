...
(function(root){
 ...
 const rpcAllow=new Set(['crm_site_address_update_v1','crm_site_identity_relink_v1','crm_site_record_relink_v1','crm_site_record_corrected_separate_v1','crm_site_admin_search_v1','crm_site_linked_history_v1','crm_site_linked_assets_v1','crm_site_link_review_list_v1','crm_site_link_review_resolve_v1','crm_site_record_link_review_list_v1','crm_site_record_link_review_resolve_v1','crm_orphan_organization_history','crm_manager_request_create_v1','crm_manager_request_list_v1','crm_profile_scoped_v2','crm_read_scoped_v2','crm_contacts_scoped_v2','crm_write_command_v2','crm_operational_source_v1','crm_expansion_note','crm_expansion_context']);
 ...
 async function read(resource,args={}){
  ...
  if(resource==='operational'){
   ...
   const knownDomains=['deal_core','inquiry_core','expansion_pool','customer_support_action','message_log','campaign_core'],domains=args.domains===undefined?knownDomains:args.domains;
   ...
   const rows=Object.fromEntries(await Promise.all(domains.map(async domain=>[domain,await collect(domain)]))),deals=rows.deal_core||[],inquiries=rows.inquiry_core||[],expansion_pool=rows.expansion_pool||[],customer_support_actions=rows.customer_support_action||[],message_logs=rows.message_log||[],campaigns=rows.campaign_core||[];
   return {contract_version:1,resource,coverage:truncated.size?'recent_window_for_requested_domains':domains.length===knownDomains.length?'complete_for_actor_scope':'complete_for_requested_domains',scope:'actor_authorized_rows_only',data:{contract_version:6,loaded_domains:domains.slice(),truncated_domains:[...truncated],deals,inquiries,expansion_pool,customer_support_actions,customerSupportActions:customer_support_actions,message_logs,messageLogs:message_logs,campaigns,campaign_logs:campaigns,expansion_events:expansion_pool.flatMap(x=>Array.isArray(x.events)?x.events:[])}};
  }
