(function(root){
 ...
 function elapsed(q){
  ...
  return {minutes:minutes,label:Math.floor(hours/24)+'일 지남'};
 }
 function evidence(q){
 ...
 root.todayInquiryCompactCard=function(x){
  var q=x.item||{},index=root.TODAY_ADMIN_INQUIRY.indexOf(x),site=q.site||q.site_name||(q.detail&&q.detail.site)||'현장명 미입력',created=receivedAt(q),meta=x.unassigned?root.inquiryUnassignedMeta(q):null,key=root.inqKey(q);
  var evidence=meta?'<span class="today-assignment-evidence">'+root.esc(meta.label)+' · '+root.esc(meta.attemptLabel)+'</span>':'';
  var action=x.unassigned?'<button class="today-inquiry-action" data-k="'+root.escAttr(key)+'" onclick="event.stopPropagation();todayAssignInquiry(this.dataset.k)">담당자 배정</button>':'<span>열기 →</span>';
  return '<div class="today-work-card today-inquiry-compact '+x.tone+'" role="button" tabindex="0" onclick="todayOpenWork(\'admin-inquiry\','+index+')" onkeydown="todayInquiryKey(event,'+index+')"><span class="today-work-main"><strong>'+root.esc(site)+'</strong><small>'+root.esc(x.owner)+' · '+root.esc(x.stage)+' · 접수 '+root.esc(created?root.dateTimeLabel(created):'일시 미확인')+'</small><em>'+root.esc(x.reason)+'</em>'+evidence+'<span>'+root.esc(x.recent)+'</span><b>다음 조치 · '+root.esc(x.next)+'</b></span><span class="today-inquiry-quick">'+action+'</span></div>';
 };
 root.todayInquiryBoard=function(title,desc,rows){
  var allowed=['all','unassigned','unanswered','quote','material','dispatch','delayed'],active=allowed.indexOf(root.G.todayInquiryFilter)>=0?root.G.todayInquiryFilter:'all',shown=rows.filter(function(x){return root.todayInquiryFilterMatch(x,active)}),visible=shown.slice(0,root.TODAY_ADMIN_VISIBLE_LIMIT);
  return '<section class="today-board inquiry"><header><div><span>📥 '+title+'</span><p>'+root.esc(desc)+' · 지금 개입할 순서만 보여줍니다.</p></div><b>'+rows.length+'건</b></header>'+root.todayInquiryFilterBar(rows,active)+'<div class="today-board-list">'+(visible.map(root.todayInquiryCompactCard).join('')||'<div class="today-inquiry-filter-empty">현재 선택한 조건의 견적문의가 없습니다.</div>')+'</div>'+root.todayBoardFoot(shown.length,visible.length,'inquiry')+'</section>';
 };
 ...
 root.inqCtlConsole=function(Q,c){
  ...
  var summary=(admin?[['신규','이번 주 접수','신규 문의'],['미배정','원인 확인·즉시 배정','risk'],['배정완료','첫 연락 대기','warn'],['영업전환','영업건으로 전환','ok'],['휴지통','30일 보관','risk']]:[['내 할 일','연락·다음 할 일 필요','risk'],['배정완료','첫 연락 시작','warn'],['응대중','다음 할 일 관리',''],['영업전환','영업건으로 전환','ok'],['보류','재개 전 대기','']]).map(function(x){return '<button class="'+(x[2]||'')+'" onclick="inqCtlSetTab(\''+x[0]+'\')"><span>'+x[0]+'</span><b>'+c[x[0]]+'건</b><small>'+x[1]+'</small></button>'}).join('');
  ...
  var rows=root.INQ_CONSOLE_CACHE.map(function(q){
   ...
   var response='<span class="inq-ctl-response '+(late?'late':'')+'"><strong>'+(responseAt?'첫 연락 완료':assigned?'첫 연락 없음':'첫 연락 전')+'</strong><small>'+(responseAt?root.esc(root.inqCtlResponseLabel(responseAt)):(late?'배정 후 '+root.INQUIRY_RESPONSE_SLA_HOURS+'시간 초과':root.esc(meta?meta.elapsed.label+' 경과':'미배정')))+'</small></span>';
  ...
  var header=admin?headerLead+'<span>접수</span><span>현장·연락처·사업유형</span><span>공종</span><span>상담담당</span><span>영업담당·미배정 사유</span><span>첫 연락</span><span>상태·다음 할 일</span><span>바로 조치</span>':headerLead+'<span>접수</span><span>현장·연락처·사업유형</span><span>공종</span><span>첫 연락</span><span>상태·다음 할 일</span><span>처리</span>';
