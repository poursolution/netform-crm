...
(function(root){
 ...
 function paint(){
  ...
  if(ctx.mobile)openSheet('단계별 정보 입력',html);else{
   let host=document.getElementById('inlineTransition'),modal=document.getElementById('stageTransitionModal');if(!host){modal=document.createElement('div');modal.id='stageTransitionModal';modal.className='stage-transition-overlay';modal.setAttribute('role','dialog');modal.setAttribute('aria-modal','true');modal.innerHTML='<section class="stage-transition-dialog"><div id="inlineTransition"></div></section>';document.body.appendChild(modal);host=document.getElementById('inlineTransition')}host.innerHTML=html;if(modal)document.body.style.overflow='hidden';
  }
