...
BEGIN
  ...
  IF matched_inquiry.assigned_to IS NOT NULL THEN
    ...
    THEN
      RAISE EXCEPTION 'assignment_sync_rejected:EXISTING_UUID_PRESERVED'
        USING ERRCODE = 'P0001';
    END IF;
  END IF;
...
'n8n 견적문의 배정 완료 전 assigned_to UUID를 동기화한다. 동기화 거부는 오류로 반환하여 완료 알림이 나가지 않게 하며, 재배정은 기존 담당자 CAS 조건을 요구한다.';
