...
(function(w){
 ...
 function mount(root,query=''){
  ...
  function renderToolbar(){const specs=[['all','전체',items.length],['candidate','기존 Site 후보',items.filter(x=>(x.site_candidates||[]).length).length],['separate','별도 현장 후보',items.filter(x=>!(x.site_candidates||[]).length).length]];toolbar.replaceChildren(...specs.map(x=>{const b=node('button',x[1]+' '+x[2],filter===x[0]?'on':'');b.type='button';b.onclick=()=>{filter=x[0];page=1;render();};return b;}));}
  function render(){renderToolbar();const filtered=items.filter(x=>filter==='all'||filter==='candidate'&&(x.site_candidates||[]).length||filter==='separate'&&!(x.site_candidates||[]).length),pages=Math.max(1,Math.ceil(filtered.length/PAGE_SIZE));page=Math.min(page,pages);const rows=filtered.slice((page-1)*PAGE_SIZE,page*PAGE_SIZE);if(!rows.length){message(list,search?'검색·필터 조건에 맞는 연결 검토 건이 없습니다.':'연결 확인이 필요한 과거자료가 없습니다.');pager.replaceChildren();return;}const frag=document.createDocumentFragment();
   rows.forEach(row=>{
    ...
    const link=node('button','선택 Site에 연결'),searchButton=node('button','다른 Site 찾기','secondary'),separate=node('button','별도 현장','secondary');link.type=searchButton.type=separate.type='button';link.disabled=!(row.site_candidates||[]).length;
    async function resolve(resolution,site){const assets='과거 메모 '+Number(row.note_count||0)+'건 · 담당자 '+contacts+'명',label=resolution==='linked'?'선택한 Site에 '+assets+'을 연결':'새 canonical Site를 만들고 '+assets+'을 연결';if(!w.confirm(label+'하시겠습니까?'))return;link.disabled=separate.disabled=true;try{const ack=await transport.rpc('crm_site_link_review_resolve_v1',{p_organization:row.organization_id,p_resolution:resolution,p_site:site||null});if(ack?.ok!==true||!ack.site_id)throw Error('ACK_MISMATCH');assetActor='';w.toast(resolution==='separate'?'새 Site를 생성하고 과거자료를 연결했습니다.':'과거자료 연결을 확정했습니다.');await refresh();if(typeof w.paintSites==='function')w.paintSites();}catch(e){link.disabled=!select.value;separate.disabled=false;w.toast('연결 결과를 확인하지 못했습니다. 다시 시도해 주세요.');}}
