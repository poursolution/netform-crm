(function(root){
 ...
 function executionRow(x,admin,rank){
  const site=x.item.site||x.item.site_name||'현장명 미입력',meta=actionMeta(x,admin),due=elapsed(x);
  return '<article class="twx-row '+(x.urgent?'urgent':'')+'" data-key="'+attr(x.key)+'">'+(rank?'<b class="twx-rank">'+rank+'</b>':'')+'<div class="twx-site"><strong>'+h(site)+'</strong><span>'+h(TYPES[x.kind])+'</span><small>'+h(x.owner)+' · '+h(x.stage)+'</small></div><div class="twx-task"><span>해야 할 일</span><strong>'+h(x.next)+'</strong><small>'+h(x.reason)+'</small></div><div class="twx-why"><span>최근 활동</span><b>'+h(x.recent)+'</b></div><div class="twx-due"><span>기한</span><b>'+h(due)+'</b></div><div class="twx-actions"><button data-key="'+attr(x.key)+'" data-action="'+meta.action+'" onclick="TodayWorkQueue.open(this.dataset.key,this.dataset.action)">'+meta.label+'</button><button class="quiet" data-key="'+attr(x.key)+'" onclick="TodayWorkQueue.open(this.dataset.key)">상세</button></div></article>';
 }
 function executionSection(key,title,description,rows,admin,ranked){
  const visible=rows.slice(0,SIZE);
  return '<section class="twx-section '+key+'" aria-label="'+h(title)+'"><header><div><span>'+h(key==='priority'?'PRIORITY':'TODAY')+'</span><h3>'+h(title)+'</h3><p>'+h(description)+'</p></div><b>'+rows.length+'건</b></header><div class="twx-list">'+(visible.map((x,i)=>executionRow(x,admin,ranked?i+1:null)).join('')||'<p class="twx-empty">현재 이 구역에서 처리할 업무가 없습니다.</p>')+'</div>'+(rows.length>SIZE?'<footer>우선 '+SIZE+'건 표시 · 검색이나 상태 필터로 범위를 좁혀주세요.</footer>':'')+'</section>';
 }
 function domainSummary(rows){
  const domains=[['inquiry','견적문의'],['pipeline','영업'],['expansion','영업지원'],['relationship','관계관리']];
  return '<section class="twx-domains"><header><h3>오늘 관리해야 할 업무</h3><p>왜 오늘 관리대상인지 함께 표시합니다.</p></header><div>'+domains.map(([key,label])=>{const list=rows.filter(x=>x.kind===key),signals=[],overdue=list.filter(x=>x.overdue).length,missing=list.filter(x=>x.missingNext).length,unassigned=list.filter(x=>x.unassigned).length,stale=list.filter(x=>x.stale).length;if(unassigned)signals.push('미배정 '+unassigned);if(overdue)signals.push('기한초과 '+overdue);if(missing)signals.push('Next 없음 '+missing);if(stale)signals.push('장기정체 '+stale);return '<article><span>'+label+'</span><b>'+list.length+'건</b><small>'+h(signals.join(' · ')||'현재 관리 예외 없음')+'</small><button onclick="TodayWorkQueue.set(\'type\',\''+key+'\')">목록 보기</button></article>'}).join('')+'</div></section>';
 }
 function render(){
  ...
  const exceptionRows=rows.filter(x=>x.urgent||x.overdue||x.unassigned||x.missingNext||x.stale||x.kind==='manager'),todayRows=rows.filter(x=>x.dueDays===0&&!exceptionRows.includes(x));
  const content=X.admin?executionSection('priority','관리자 개입 우선순위','미배정·응대지연·기한초과·Next 누락·장기정체',exceptionRows,true,true)+domainSummary(exceptionRows):executionSection('priority','지금 먼저 처리하세요','기한초과·고객응답 후 미처리·Next Action 없음',exceptionRows,false,true)+executionSection('schedule','오늘 예정 업무','오늘 예정된 연락·방문·미팅·견적 후속',todayRows,false,false);
  host.innerHTML='<div class="today-work-queue execution '+(X.admin?'manager':'rep')+'"><header><div><span>'+(X.admin?'MANAGER INTERVENTION':'MY ACTION DESK')+'</span><h2>오늘 업무</h2><p>'+(X.admin?'관리자가 실제로 개입해야 할 순서와 영역별 이유입니다.':'우선 처리할 일과 오늘 예정된 내 업무입니다.')+'</p></div><b>'+(exceptionRows.length+todayRows.length)+'건</b></header>'+counters+toolbar+'<p class="twq-count-note">업무유형은 보조 배지로만 표시합니다.</p>'+content+'</div>';
