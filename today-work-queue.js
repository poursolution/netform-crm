(function(root){
 ...
 function resetForActor(admin){
  ...
  if(actor!==currentActor){currentActor=actor;root.G.todayQueueFilter='all';root.G.todayQueueOwner='전체';root.G.todayQueueType='all';root.G.todayQueueSearch='';root.G.todayQueuePage=1;root.G.todayRoutinePage=1}
 }
 function set(field,value){
  ...
  root.G[allowed[field]]=value;root.G.todayQueuePage=1;root.G.todayRoutinePage=1;render();
 }
 function page(n,routine){root.G[routine?'todayRoutinePage':'todayQueuePage']=Math.max(1,Number(n)||1);render()}
 ...
 function table(rows,admin,routine){
  const name=routine?'todayRoutinePage':'todayQueuePage',pages=Math.max(1,Math.ceil(rows.length/SIZE)),n=Math.min(Math.max(1,Number(root.G[name])||1),pages);root.G[name]=n;
  const start=(n-1)*SIZE,shown=rows.slice(start,start+SIZE);
  const pagesHtml=Array.from({length:pages},(_,i)=>i+1).filter(i=>i===1||i===pages||Math.abs(i-n)<=2).map((i,j,all)=>(j&&i>all[j-1]+1?'<span>…</span>':'')+'<button '+(i===n?'aria-current="page"':'')+' onclick="TodayWorkQueue.page('+i+','+!!routine+')">'+i+'</button>').join('');
  return '<section class="twq-list '+(routine?'today-rep-routine':admin?'today-admin':'today-rep-priority')+'" aria-label="'+(routine?'오늘 예정 업무':'오늘 처리 순서')+'">'+(routine?'<h3>오늘 업무 · '+rows.length+'건 <small>상단 우선순위와 중복 제외</small></h3>':'')+'<table><colgroup><col style="width:5%"><col style="width:10%"><col style="width:24%"><col style="width:9%"><col style="width:32%"><col style="width:10%"><col style="width:10%"></colgroup><thead><tr>'+['우선','유형','현장','담당자','지금 해야 할 일','기한/경과','처리'].map(x=>'<th scope="col">'+x+'</th>').join('')+'</tr></thead><tbody>'+shown.map((x,i)=>row(x,start+i+1,admin)).join('')+'</tbody></table>'+(!rows.length?'<p class="twq-empty">현재 조건에서 처리할 업무가 없습니다.</p>':'')+'<footer><span>'+rows.length+'건 · 페이지당 '+SIZE+'건</span><nav aria-label="'+(routine?'예정 업무':'우선순위')+' 페이지"><button '+(n===1?'disabled':'')+' onclick="TodayWorkQueue.page('+(n-1)+','+!!routine+')">이전</button>'+pagesHtml+'<button '+(n===pages?'disabled':'')+' onclick="TodayWorkQueue.page('+(n+1)+','+!!routine+')">다음</button></nav></footer></section>';
 }
 function render(){
  ...
  host.innerHTML='<div class="today-work-queue '+(X.admin?'manager':'rep')+'">'+(!X.admin?'<h2>오늘 우선순위</h2>':'')+counters+toolbar+'<p class="twq-count-note">선택 '+rows.length+'건 · 긴급/기한초과/미배정/Next 없음은 중복될 수 있습니다.</p>'+table(priority,X.admin,false)+(!X.admin?table(routine,false,true):'')+'</div>';
