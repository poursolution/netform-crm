(function(root){
 ...
 function people(s,limit){
  const names=[...new Set(s.deals.map(x=>x.owner))].filter(n=>root.SalesScope.people().some(p=>p.name===n)).sort(root.repCompare);
 ...
 function render(){
  ...
  host.innerHTML='<div class="si-shell">'+filters()+(page==='perf'?'<div class="si-views" role="group" aria-label="분석 관점">'+btn('대표 보기','view','lead',f.view==='lead'?'selected':'')+btn('영업사원 보기','view','rep',rep?'selected':'')+'</div>':'')+'<p class="si-period">'+h(f.year)+'년 '+(f.month?f.month+'월':'연간')+' 접수·수주 / 파이프라인·관리필요는 현재 기준'+(s.missingWonDate?' · 수주 확정일 미입력 '+s.missingWonDate+'건 제외':'')+'</p>'+body+'</div>';
