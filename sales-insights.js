(function(root){
 ...
 function kpis(s,rep){
  const defs=rep?[['진행현장',number(s.active.length)+'건','현재','active'],['예상금액',money(s.expected),'현재 진행 중','active'],['수주금액',money(s.wonAmount),'선택 기간 · 확정 금액','won'],['미접촉',number(s.active.filter(d=>d.issues.includes('contact')).length)+'건','최근 접촉 7일 이상','contact'],['Next 없음',number(s.active.filter(d=>d.issues.includes('missing')).length)+'건','행동 또는 기한 미입력','missing']]:[['문의',number(s.inquiries.length)+'건','선택 기간 접수','inquiries'],['진행중',number(s.active.length)+'건','현재 보유 파이프라인','active'],['예상 파이프라인 금액',money(s.expected),'현재 진행 중','active'],['수주금액',money(s.wonAmount),'선택 기간 · 확정 금액','won'],['관리필요',number(s.risk.length)+'건','중복 제외 현장 수','risk']];
 ...
 function risks(s){return card('지금 관리가 필요한 것','<div class="si-risk-links">'+Object.keys(labels).map(k=>btn(labels[k]+' '+number(s.active.filter(d=>d.issues.includes(k)).length)+'건','drill',k)).join('')+'</div>'+btn('전체 확인 →','drill','risk'),'한 현장에 여러 사유가 있을 수 있습니다.');}
 ...
 function render(){
  ...
  else body=kpis(s,rep)+'<div class="si-grid">'+stages(s)+(page==='dash'?risks(s):rep?execution(s):trend(s))+'</div>'+(rep?card('현재 관리가 필요한 현장 · '+s.risk.length+'건',records(s.risk,8)+btn('전체 확인 →','drill','risk'))+recent(s):people(s,page==='dash'?8:0));
 ...
 function openPerson(name,trigger){
  ...
  shade.id='si-person';shade.className='modalshade on si-person';shade.innerHTML='<section role="dialog" aria-modal="true" aria-labelledby="si-person-title" class="si-person-box"><header><div><h2 id="si-person-title">'+h(name)+' 영업 현황</h2><p>'+h(state().year)+'년 '+(state().month?state().month+'월':'연간')+' · '+h(state().brand)+'</p></div>'+btn('닫기','close','','si-close')+'</header><div class="si-person-grid">'+card('담당자 요약','<dl class="si-facts"><div><dt>진행현장</dt><dd>'+s.active.length+'건</dd></div><div><dt>예상금액</dt><dd>'+money(s.expected)+'</dd></div><div><dt>기간 수주</dt><dd>'+money(s.wonAmount)+'</dd></div><div><dt>수주 건수</dt><dd>'+s.won.length+'건</dd></div></dl>')+'<div>'+stages(s)+execution(s)+'</div>'+card('관리 필요 · '+s.risk.length+'건',records(s.risk))+'</div></section>';
