...
(function(w){
 ...
 const ACTS=/^(da:(activity|next|stage|support)|dcc|brief)$/;
 w.CRM_SOLO=SOLO;
 ...
 function applyAct(act,delay){if(!act||!ACTS.test(act))return;const run=()=>{try{if(act.startsWith('da:'))origDA?.call(w.DetailActions,act.slice(3));else if(act==='dcc')origDcc?.call(w);else if(act==='brief')origBrief?.call(w);}catch(e){}};delay?setTimeout(run,delay):run();}
 ...
 if(typeof baseLoad==='function')w.loadData=async function(){
  ...
  if(SOLO&&!actDone&&w.CUR_DETAIL){actDone=true;applyAct(params.get('act'),300);}
  return result;
 };

 /* 안전장치: 이 모듈보다 데이터가 먼저 들어온 경우 — 주소에 상세 요청이 있으면 준비될 때까지 잠깐 다시 확인한다(최대 30초) */
 if(params.get('detail')||params.get('inquiry')){let tries=0;const t=setInterval(()=>{tries++;const done=!!w.CUR_DETAIL||!!document.getElementById('inq-inbox-dialog');if(!done&&w.B){try{w.openRequestedDetail?.();}catch(e){}}if(SOLO&&!actDone&&w.CUR_DETAIL){actDone=true;applyAct(params.get('act'),300);}if(done||tries>75)clearInterval(t);},400);}

 if(!SOLO){w.DetailWindow=Object.freeze({open:openWindow,allowed});return;}
 ...
 w.DetailWindow=Object.freeze({open:openWindow,allowed,solo:true});
})(window);
