...
공급자 상태 9개를 홈과 이력에 동일하게 표시한다. 일부 성공과 미확정 결과를 대기로 숨기지 않으며, 진행 중과 결과 확인 필요 건수를 따로 제공한다. 각 캠페인의 성공·실패 수신자 수를 표시하고 성과는 전체 대상 수가 아닌 서버의 sent_count를 집계한다. 과거 sent_count 없는 완료 이력에만 기존 대상 수를 사용한다. 미확정 건은 자동 재발송하지 않는다.

신규 회귀 테스트 7개로 상태별 표시, 일부 완료 성과, 명시적 0, 과거 이력 호환, 알 수 없는 상태의 안전 표시 및 연도·예약 필터를 확인했다. CRM 계약 165개와 알리고/Postgres 테스트 26개, 로컬 배포 산출물 검증이 통과했다. 실제 렌더 함수에 합성 데이터를 넣은 로컬 브라우저 화면에서 상태와 성공·실패 표시를 확인했다. 이 확인은 운영 수신자 발송이나 인증된 운영 화면 E2E 검증을 의미하지 않는다.

GitHub 계정 netformrndsvcteams-jpg는 poursolution/netform-crm의 공동작업자 초대를 수락했고 수정 권한을 확인했다. Supabase와 같은 계정으로 저장소 작업이 가능하며 소유자와 운영 URL은 유지한다. 서버 인증 저장·worker 실행의 미완료 상태는 별도다.
