...
|---:|---|---|---|
| P0 | 기존 인증 계정의 휴대폰 번호 기반 비밀번호 잔존 가능성 | PC·모바일 소스는 개인 비밀번호 안내와 원문 입력으로 수정했지만, Production Auth 계정의 실제 비밀번호가 회전됐다는 증거는 없다. | 기존 계정 강제 재설정, 개인별 임시 비밀번호 발급, 초기 변경, 재설정, 퇴사자 회수 흐름 적용 |
...
- PC·모바일 로그인 화면에서 휴대폰 번호를 비밀번호로 쓰라는 안내와 숫자 전용 입력을 제거했다. 비밀번호 원문을 Supabase Auth에 전달하며, 이를 고정하는 계약 테스트 5개를 추가했다.
...
1. Production Auth의 기존 계정을 점검해 휴대폰 번호 기반 비밀번호를 강제 재설정하고 발급·초기 변경·재설정·퇴사자 회수 절차를 적용한다.
2. 관계관리 원자 저장과 데이터 정리 RPC를 테스트 계정/테스트 레코드로 검증한다.
3. 구글시트 담당자 동기화의 실제 실행 위치를 찾는다.
4. Site 연결 Queue와 고객자산 미연결 데이터를 사람 검토 방식으로 정리한다.
5. 문자 provider와 Campaign callback을 테스트 수신자로 E2E 검증한다.
