...
- 컨설팅은 Triage 표, 자료발송은 Follow-up Split, 경쟁입찰은 일정판, 계약시공은 운영표, 수주·실주는 결과 요약과 목록을 사용한다. 자료발송 외에는 현장 선택 시에만 공통 Inspector를 열고 닫으면 전체 폭 목록을 복원한다.
...
| 공통 Queue 렌더링 | `stage-workspaces.js` | Spec의 workspaceType에 따라 Triage/Table/Schedule/Queue를 조합한다. 상세 및 입력 폼은 복제하지 않는다. |
...
- 컨설팅은 오늘 처리 기본, 한 페이지 30건이다. 기한 도래·초과, 3일 이내 견적, 최근 7일 생성만 오늘 처리에 포함하며 과거 누락만으로 활성화하지 않는다. 날짜 없는 과거 자료를 임의 대기 상태로 저장하지 않는다. 나머지 초기 목록은 60건만 렌더링하고 더 보기로 확장한다. 전체 결과 건수와 표시 건수를 구분한다.
...
화면 간격·폰트 조정은 이 구조 이후에 진행한다. 업무에 맞는 레이아웃을 고르되 공통 상세 Renderer와 입력·저장 경로는 재사용한다.

## 컨설팅 일괄처리

- 현재 표에서 직접 선택한 행만 처리하고 필터·페이지 변경 시 선택을 초기화한다.
- 공종은 기존 공종을 보존하여 추가, 다음 확인일은 기존 다음 행동 교체, 대기 전환은 기존 waiting 명령을 사용한다.
- 선택 시 version과 서버 read version을 대조하고 순차 ACK 후에만 화면을 반영한다. 충돌·미확인·계정 변경 시 중단하고 성공 건수와 잔여 중단을 구분한다.
- 담당자 일괄배정은 opportunity assign 서버 명령이 연결되지 않아 미제공이다. 별도 보류 상태도 임의 생성하지 않는다. 이 두 기능의 서버 계약은 후속 작업이다.
