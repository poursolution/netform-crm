-- EXACT DISPOSABLE FIXTURE CLEANUP. REVIEW AND APPROVE BEFORE EXECUTION.
-- Project: netform-crm-staging / rprechiaglyjaydkmxsu
-- Run: stg-e2e-20260906t125239z-fc70f2d2
-- Scope: 41 preallocated entity UUIDs + 67 actually executed request UUIDs.
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE d.list_fields->>'fixture_run_id' IS DISTINCT FROM 'stg-e2e-20260906t125239z-fc70f2d2'
  ...
  IF EXISTS (
    ...
    WHERE i.raw->>'fixture_run_id' IS DISTINCT FROM 'stg-e2e-20260906t125239z-fc70f2d2'
...
DO $verify$
BEGIN
  IF EXISTS (SELECT 1 FROM public.deals WHERE list_fields->>'fixture_run_id'='stg-e2e-20260906t125239z-fc70f2d2')
     OR EXISTS (SELECT 1 FROM public.inquiries WHERE raw->>'fixture_run_id'='stg-e2e-20260906t125239z-fc70f2d2')
     OR EXISTS (SELECT 1 FROM crm_security.object_scope WHERE reviewed_by='fixture:stg-e2e-20260906t125239z-fc70f2d2')
