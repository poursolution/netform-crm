...
function periodSeg(rows){
 ...
 const ins=['dash','control','perf'].includes(root.G.page)&&root.G.insights;
 const G=ins?root.G.insights:root.G,years=[...new Set(rows.map(r=>String(r.item?.created||'').slice(0,4)).filter(y=>/^20/.test(y)))].sort().reverse();
 if(!years.length)years.push(String(new Date().getFullYear()));
 const y='<select class="uni-year" aria-label="연도" onchange="uniSetYear(this.value)">'+['전체',...years].map(v=>'<option value="'+a(v)+'"'+(String(G.year)===String(v)?' selected':'')+'>'+h(v==='전체'?'전체 연도':v+'년')+'</option>').join('')+'</select>';
 const q='<span class="uni-qseg" role="group" aria-label="기간">'+[[0,'연간'],[1,'1분기'],[2,'2분기'],[3,'3분기'],[4,'4분기']].map(x=>'<button type="button" class="'+(Number(G.quarter)===x[0]&&!G.month?'on':'')+'" onclick="uniSetQuarter('+x[0]+')">'+x[1]+'</button>').join('')+'</span>';
 return '<div class="sf-period">'+y+q+'</div>';
}
