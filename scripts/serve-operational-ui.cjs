...
const root=path.resolve(__dirname,'../staging-operational');
...
http.createServer((req,res)=>{
 const u=new URL(req.url,'http://127.0.0.1:4180'),file=u.pathname==='/'?'/crm.html':u.pathname;
 ...
 const target=path.join(root,file);if(!target.startsWith(root+path.sep)||!fs.existsSync(target)){res.writeHead(404);res.end();return;}
...
}).listen(4180,'127.0.0.1',()=>console.log('Operational UI candidate: http://127.0.0.1:4180'));
