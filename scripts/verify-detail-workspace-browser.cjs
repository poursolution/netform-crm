...
async function run(){
 ...
 try{
  ...
  await page.evaluate(()=>detailTabFocus('영업활동',true));
  ...
  assert.equal(await page.locator('#detailView').evaluate(n=>n.classList.contains('dw-quick')),true);
  assert.equal(await page.locator('#dv-act-note').isVisible(),true);
  assert.equal(await page.locator('#dv-na-date').isVisible(),true);
  assert.equal(await page.locator('.journey-wrap').isVisible(),false);
  await page.locator('#dv-act-note').fill('보존할 미저장 연락 메모');
  await page.screenshot({path:path.resolve(root,'../detail-workspace-quick.png'),animations:'disabled'});
  await page.getByRole('button',{name:'전체 영업정보 보기 →',exact:true}).click();
  assert.equal(await page.locator('#dv-act-note').inputValue(),'보존할 미저장 연락 메모');
  assert.equal(await page.locator('.journey-wrap').isVisible(),true);
