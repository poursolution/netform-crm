...
await page.evaluate(()=>{SalesScope.change('type','all');SalesScope.change('owner','전체');SalesFilterState.selectBrand('전체');B.deals=Array.from({length:125},(_,i)=>({id:'batch-'+i,site:'검증 '+i,assignee:'황윤선',brand:'POUR솔루션',code:'consulting',stage_code:'consulting',created:'2026-09-20'}));PipelineWorkspace.open('consulting');});
assert.equal(await page.locator('.ps-split-queue .sw-card').count(),60);
assert.match(await page.locator('.ps-queue-count').innerText(),/60 \/ 125/);
await page.locator('[data-ps-action="queue-more"]').click();
assert.equal(await page.locator('.ps-split-queue .sw-card').count(),120);
