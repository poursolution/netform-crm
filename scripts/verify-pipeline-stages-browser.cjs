...
assert.equal(await page.locator('#pipeline-stage-menu button').count(),8);assert.equal(await page.locator('.ps-board>section').count(),8);assert.deepEqual(await page.locator('.ps-board header').allTextContents(),['01 컨설팅 설계단계 · 0건','02 자료 발송완료 · 1건','03 관계관리 · 1건','04 경쟁·임박·입찰 · 0건','05 계약·시공 · 0건','06 수주 · 1건','07 실주 · 0건','08 확장관리 · 1건']);
await page.locator('.ps-view').getByRole('button',{name:'목록',exact:true}).click();
assert.equal(await page.locator('.ps-table-scroll tbody tr').count(),4);
assert.equal(await page.locator('.ps-management').count(),4);
assert.equal(await page.locator('.ps-management button').evaluateAll(ns=>ns.every(n=>n.getBoundingClientRect().width===60&&n.getBoundingClientRect().height===34)),true);
assert.equal(await page.locator('.ps-due').evaluateAll(ns=>ns.every(n=>n.getBoundingClientRect().width===84)),true);
assert.equal(await page.locator('.ps-metrics').evaluate(n=>n.getBoundingClientRect().bottom<=document.querySelector('.ps-filters').getBoundingClientRect().top),true);
await page.locator('.ps-view').getByRole('button',{name:'보드',exact:true}).click();
await page.locator('#pipeline-stage-menu [data-value="sent"]').click();await page.locator('.menu [data-p="pipe"]').click();assert.equal(await page.evaluate(()=>G.pipelineStage),'all');assert.equal(await page.locator('#pipeline-stage-menu [data-value="all"]').count(),0);await page.locator('#pipeline-stage-menu [data-value="sent"]').click();assert.equal(await page.locator('.sw-card').count(),1);assert.match(await page.locator('.sw-workspace').innerText(),/발송 후속 현장/);assert.equal(await page.locator('#pipeline-stage-menu .selected').getAttribute('aria-current'),'page');await page.locator('.sw-card [data-ps-action="record"]').first().click();assert.equal(await page.evaluate(()=>window.__selected),'sent-1');
await page.evaluate(()=>{const d=B.deals[0];d.code=d.stage_code='rapport';d.stageHistory.push({from:'sent',to:'rapport',at:'2026-09-20'});PipelineWorkspace.refresh();});assert.equal(await page.locator('.sw-card').count(),0);assert.match(await page.locator('#pipeline-stage-menu [data-value="relationship"]').innerText(),/2$/);await page.locator('#pipeline-stage-menu [data-value="relationship"]').click();assert.equal(await page.locator('.sw-card').count(),2);assert.equal(await page.evaluate(()=>B.deals[0].stageHistory.length),2);
await page.evaluate(()=>goPage('dash'));assert.equal(await page.locator('#si-dash [data-si-action="stage"]').count(),8);await page.locator('#si-dash [data-sales-scope="owner"]').selectOption('이필선');await page.locator('#si-dash [data-si-action="stage"][data-value="relationship"]').click();assert.equal(await page.locator('#pipeline-stage-root [data-sales-scope="owner"]').inputValue(),'이필선');assert.equal(await page.locator('.sw-card').count(),1);
...
console.log('PASS pipeline stages: sidebar, eight groups, dedicated queues, shared updates, history, dashboard filters, pending ACK, role boundary and responsive layout');
