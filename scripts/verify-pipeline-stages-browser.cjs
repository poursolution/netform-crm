...
await page.evaluate(()=>{const d=B.deals[0];d.code=d.stage_code='rapport';d.stageHistory.push({from:'sent',to:'rapport',at:'2026-09-20'});PipelineWorkspace.refresh();});assert.equal(await page.locator('.sw-work-table tbody tr[data-deal]').count(),0);assert.match(await page.locator('#pipeline-stage-menu [data-value="relationship"]').innerText(),/2$/);await page.locator('#pipeline-stage-menu [data-value="relationship"]').click();assert.equal(await page.locator('.sw-card').count(),2);assert.equal(await page.evaluate(()=>B.deals[0].stageHistory.length),2);
