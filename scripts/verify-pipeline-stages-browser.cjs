...
await page.evaluate(()=>{ME={id:'rep',name:'김성민',role:'rep'};window.__selected=null;});await page.locator('.sw-work-table [data-ps-action="record"]').first().click();assert.equal(await page.evaluate(()=>window.__selected),null);await page.evaluate(()=>PipelineWorkspace.open('all'));assert.equal(await page.evaluate(()=>PipelineWorkspace.rows().every(x=>x.owner==='김성민')),true);assert.deepEqual(await page.evaluate(()=>__writes),[]);assert.deepEqual(errors,[]);
