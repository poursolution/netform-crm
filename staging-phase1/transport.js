/* Staging common transport only. No business bundle, no Legacy fallback. */
(function(root){
 'use strict';
 const REF='rprechiaglyjaydkmxsu',VERSION=1,c=root.PHASE1_CONFIG;
 if(!c||c.project_ref!==REF||c.url!==`https://${REF}.supabase.co`||!c.publishable_key||!['127.0.0.1','localhost'].includes(location.hostname))throw Error('PHASE1_WRONG_ENVIRONMENT');
 const origin=new URL(c.url).origin,base=`crm:staging:${REF}:v${VERSION}:`,nativeLocal=root.localStorage,nativeSession=root.sessionStorage;
 ...
 const rpcAllow=new Set(['crm_profile_scoped_v2','crm_read_scoped_v2','crm_contacts_scoped_v2','crm_write_command_v2']);
 function allowed(input){const u=new URL(typeof input==='string'?input:input.url||String(input),location.href);
  ...
  if(u.origin===origin&&(u.pathname.startsWith('/auth/v1/')||rpcAllow.has(u.pathname.replace('/rest/v1/rpc/',''))&&u.pathname.startsWith('/rest/v1/rpc/')))return u;
 ...
 function enqueue(operation,objectId,expectedVersion,payload,requestId=crypto.randomUUID()){
  if(!profile)throw Error('AUTH_REQUIRED');if(!['opportunity_work_set','inquiry_assign'].includes(operation))throw Error('CONTRACT_UNAVAILABLE');
  if(!Number.isInteger(expectedVersion)||expectedVersion<0||!objectId||!payload)throw Error('INVALID_COMMAND');
  if(operation==='opportunity_work_set'&&Object.keys(payload).some(k=>!['primary_work','work_items','reason'].includes(k)))throw Error('INVALID_COMMAND');
  if(operation==='inquiry_assign'){if(expectedVersion!==0)throw Error('INVALID_COMMAND');payload=inquiryDirectPayload(payload);}
  const rows=list();if(rows.some(r=>r.request_id===requestId))throw Error('LOCAL_REQUEST_ID_REUSE');
  const command={request_id:requestId,operation,object_id:objectId,expected_version:expectedVersion,payload:JSON.parse(JSON.stringify(payload)),auth_uid:profile.auth_uid,user_id:profile.user_id,status:'pending',tries:0};rows.push(command);save(rows);return command;
 }
 function validateAck(a,q){if(a?.contract_version!==1||a.ok!==true||a.request_id!==q.request_id||a.operation!==q.operation||a.object_id!==q.object_id||a.actor_auth_uid!==q.auth_uid||a.actor_user_id!==q.user_id||typeof a.replayed!=='boolean')throw Error('ACK_CONTRACT_MISMATCH');
  if(q.operation==='opportunity_work_set'&&(a.previous_version!==q.expected_version||a.version!==q.expected_version+1||!a.audit_event_id))throw Error('ACK_CONTRACT_MISMATCH');
  if(q.operation==='inquiry_assign'&&(a.intent!=='direct_assign'||!a.assigned_to||typeof a.status!=='string'||typeof a.changed!=='boolean'||a.changed&&!a.inquiry_audit_event_id))throw Error('ACK_CONTRACT_MISMATCH');}
 ...
 root.Phase1=Object.freeze({config:c,createClient,admit,beginLogin,signOut,storage,mode,read,rpc,queue:{enqueue,flush,list,validateAck},get profile(){return profile;},requests,blocked,
