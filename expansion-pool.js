(function(){
 ...
 function boardCard(r){
  ...
  return '<article class="exp-board-card '+due.cls+'"><details><summary><span class="exp-board-status">'+h(F.status(r))+'</span><h3>'+h(r.site||'현장명 확인 필요')+'</h3><p>'+h(r.sourceWorkSummary||'기존 수주 공종 미기록')+' · '+h(fmtAmt(r.wonAmount))+'</p><div class="exp-board-owner"><strong>'+h(r.owner||repN(d.assignee)||'미배정')+'</strong><em>'+h(due.text)+'</em></div><small>'+h(work||'확장 후보를 확인해 주세요.')+'</small><b class="exp-board-open">업무 열기</b></summary><div class="exp-board-detail"><dl><div><dt>당시 담당</dt><dd>'+h(repN(d.assignee)||r.owner||'미배정')+'</dd></div><div><dt>준공일</dt><dd>'+h(r.completionDate||'미기록')+'</dd></div><div><dt>이력</dt><dd>'+history.length+'건</dd></div></dl><p class="exp-note">'+h(work||'확장 후보를 확인해 주세요.')+'</p>'+cardActions(r,done)+'</div></details></article>';
 }
