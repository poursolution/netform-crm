(function(){
 ...
 function managerHTML(r){
  ...
  return '<section class="exp-manage-hero '+due.cls+'"><div><span class="exp-board-status">'+h(F.status(r))+'</span><h3>'+h(r.site||'현장명 확인 필요')+'</h3><p>'+h(r.sourceWorkSummary||'기존 수주 공종 미기록')+' · 수주 '+h(fmtAmt(r.wonAmount))+'</p></div><em>'+h(due.text)+'</em></section>'+contact+'<dl class="exp-manage-facts">'+[['현재 담당',r.owner||repN(d.assignee)||'미배정'],['당시 영업담당',repN(d.assignee)||r.owner||'미배정'],['계약일',String(contract).slice(0,10)],['준공일',r.completionDate||'미기록']].map(x=>'<div><dt>'+h(x[0])+'</dt><dd>'+h(x[1])+'</dd></div>').join('')+'</dl><section class="exp-manage-signal"><span>다음 영업 신호</span><strong>'+h(work||'확장 후보를 확인해 주세요.')+'</strong></section><section class="exp-manage-history"><header><h3>접촉·니즈 이력</h3><span>'+history.length+'건</span></header><ol>'+timeline+'</ol></section><section class="exp-manage-actions"><header><h3>이 현장 관리</h3><span>변경 내용은 서버 저장 후 보드에 반영됩니다.</span></header>'+cardActions(r,done)+'</section>';
 }
 ...
 function render(){
  ...
  root.innerHTML='<section class="exp-queue-head"><div><h2>확장관리 <b>'+yearRows.length+'</b></h2><p>재접촉이 필요한 기존 고객을 관리합니다.</p></div><nav aria-label="확장관리 핵심 현황"><button onclick="G.expansionDueFilter=\'도래\';G.expansionPage=1;paintExpansion()">접촉 필요 <b>'+globalDue.length+'</b></button><span>니즈확인 <b>'+active.filter(r=>F.status(r)==='니즈확인').length+'</b></span><span>전환 <b>'+made.length+'</b></span></nav></section><nav class="exp-year-tabs" aria-label="확장관리 연도">'+yearTabs.map(y=>'<button data-v="'+h(y)+'" class="'+(String(year)===y?'on':'')+'" title="'+h(y==='이전'?(current-3)+'년 이전 거래':y+'년 거래')+'" onclick="G.expansionYear=this.dataset.v;G.expansionOwner=\'전체\';G.expansionPage=1;paintExpansion()">'+h(y)+'</button>').join('')+'</nav>'+'<details class="dw-fold"><summary>담당자별 상태 집계</summary>'+ownerSummary(yearRows)+'</details>'+'<div class="exp-toolbar"><label>담당자 <select title="선택한 담당자의 확장관리 현장만 표시합니다." onchange="G.expansionOwner=this.value;G.expansionPage=1;paintExpansion()">'+owners.map(o=>'<option '+((G.expansionOwner||'전체')===o?'selected':'')+'>'+h(o)+'</option>').join('')+'</select></label><label>상태 <select title="확장관리 상태로 화면을 좁힙니다." onchange="G.expansionStatusFilter=this.value;G.expansionPage=1;paintExpansion()">'+['전체',...F.statuses].map(s=>'<option '+(s===statusFilter?'selected':'')+'>'+h(s)+'</option>').join('')+'</select></label><label>다음 접촉일 <select title="상태 조건을 유지한 채 접촉 기한으로 한 번 더 좁힙니다." onchange="G.expansionDueFilter=this.value;G.expansionPage=1;paintExpansion()"><option value="전체" '+(dueFilter==='전체'?'selected':'')+'>전체</option><option value="도래" '+(dueFilter==='도래'?'selected':'')+'>오늘·기한초과</option></select></label><label class="exp-search">검색 <span><input value="'+h(G.expansionQuery||'')+'" placeholder="현장·담당자·공종" onkeydown="if(event.key===\'Enter\'){G.expansionQuery=this.value;G.expansionPage=1;paintExpansion()}"><button onclick="G.expansionQuery=this.previousElementSibling.value;G.expansionPage=1;paintExpansion()">검색</button></span></label><div class="exp-view-switch" role="group" aria-label="확장관리 보기 방식"><button class="'+(view==='board'?'on':'')+'" aria-pressed="'+(view==='board')+'" onclick="G.expansionView=\'board\';paintExpansion()">▦ 상태 보드</button><button class="'+(view==='list'?'on':'')+'" aria-pressed="'+(view==='list')+'" onclick="G.expansionView=\'list\';paintExpansion()">☷ 관리 목록</button></div><span class="hint">선택 결과 '+rows.length+'건'+(view==='list'?' · '+page+' / '+pageCount+'페이지':' · 상태별 분류')+'</span></div>'+content;
  root.querySelectorAll('option').forEach(n=>{if(n.textContent==='관계관리'){n.value='관계관리';n.textContent='유지접촉'}});root.querySelectorAll('.exp-board-status,.exp-board-column>header h3,.exp-owner-row.head span,.exp-pool-card>header small').forEach(n=>{if(n.textContent==='관계관리')n.textContent='유지접촉'});
 }
 function card(r){
 ...
 const due=dueMeta(r),signal=r.needNote?'확인 니즈 · '+r.needNote:'추천 · '+r.candidates.join(' · ');
 return '<article class="exp-pool-card exp-queue-row '+due.cls+'"><header><div><button class="exp-site-link" data-id="'+h(r.id)+'" onclick="ExpansionPool.open(this.dataset.id)">'+h(r.site||'현장명 확인 필요')+'</button><small>'+h(F.status(r))+(done?' · Pipeline 전환 완료':'')+'</small></div>'+button(r,'관리 열기 ↗','ExpansionPool.open',true)+'</header><dl>'+[['담당자',r.owner||repN(d.assignee)||'미배정'],['다음 접촉',due.text],['기존 수주',r.sourceWorkSummary||'공종 미기록'],['금액',fmtAmt(r.wonAmount)]].map(x=>'<div><dt>'+x[0]+'</dt><dd>'+h(x[1])+'</dd></div>').join('')+'</dl><p class="exp-note">'+h(signal||'확장 후보를 확인해 주세요.')+'</p><div class="exp-list-history">이력 '+history.length+'건 · 상세 관리와 실행은 관리창에서 진행합니다.</div></article>';
 }
