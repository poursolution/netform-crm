(function(){
 ...
 // 하단은 상태별 업무 보드(칸반) 하나 — 상단은 파이프라인과 같은 슬림 문법.
 ...
 function render(){
  ...
  const dueNow=active.filter(r=>daysTo(r.nextContactAt)!=null&&daysTo(r.nextContactAt)<=0).length,noNext=active.filter(r=>!r.nextContactAt).length;
  const kpis='<div class="ps-kpis">'+[['선택연도 거래',yearRows.length,''],['기한초과·오늘',dueNow,dueNow?'bad':''],['접촉일 미지정',noNext,noNext?'warn':''],['니즈확인',active.filter(r=>F.status(r)==='니즈확인').length,''],['Pipeline 전환',made.length,'']].map(x=>'<div class="ps-kpi'+(x[2]?' '+x[2]:'')+'"><span>'+x[0]+'</span><b>'+x[1]+'</b></div>').join('')+'</div>';
  const content=board(rows,statusFilter);
  root.innerHTML=SalesFilters.controls(sourceRows.filter(r=>F.yearMatch(r,expansionSourceDeal(r)||{},year,current)).map(r=>({brand:(expansionSourceDeal(r)||{}).brand,owner:r.owner,item:expansionSourceDeal(r)})))+kpis+'<nav class="exp-year-tabs" aria-label="확장관리 연도">'+yearTabs.map(y=>'<button data-v="'+h(y)+'" class="'+(String(year)===y?'on':'')+'" title="'+h(y==='이전'?(current-3)+'년 이전 거래':y+'년 거래')+'" onclick="G.expansionYear=this.dataset.v;G.expansionListLimit=50;paintExpansion()">'+h(y)+'</button>').join('')+'</nav>'+'<section class="sw-owner-board exp-owner-board" aria-label="담당자별 확장 현황"><header><h3>담당자별 현황</h3><small>행 클릭 = 아래 목록 필터</small></header>'+ownerSummary(yearRows)+'</section>'+'<div class="exp-toolbar"><label>상태 <select title="확장관리 상태로 화면을 좁힙니다." onchange="G.expansionStatusFilter=this.value;G.expansionListLimit=50;paintExpansion()">'+['전체',...F.statuses].map(s=>'<option '+(s===statusFilter?'selected':'')+'>'+h(s)+'</option>').join('')+'</select></label><label>다음 접촉일 <select title="상태 조건을 유지한 채 접촉 기한으로 한 번 더 좁힙니다." onchange="G.expansionDueFilter=this.value;G.expansionListLimit=50;paintExpansion()"><option value="전체" '+(dueFilter==='전체'?'selected':'')+'>전체</option><option value="도래" '+(dueFilter==='도래'?'selected':'')+'>오늘·기한초과</option></select></label><label class="exp-search">검색 <span><input value="'+h(G.expansionQuery||'')+'" placeholder="현장·담당자·공종" onkeydown="if(event.key===\'Enter\'){G.expansionQuery=this.value;G.expansionListLimit=50;paintExpansion()}"><button onclick="G.expansionQuery=this.previousElementSibling.value;G.expansionListLimit=50;paintExpansion()">검색</button></span></label><span class="hint">선택 결과 '+rows.length+'건 · 상태별 분류</span></div>'+content;
  root.querySelectorAll('option').forEach(n=>{if(n.textContent==='관계관리'){n.value='관계관리';n.textContent='유지접촉'}});root.querySelectorAll('.exp-board-status,.exp-board-column>header h3,.exp-owner-row.head span').forEach(n=>{if(n.textContent==='관계관리')n.textContent='유지접촉'});
 }
