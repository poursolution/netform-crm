(function(){
 ...
 function render(){
  ...
  root.innerHTML='<section class="exp-command"><div class="eyebrow">기존 수주고객 · 재영업 시점 관리</div><h2>과거 거래는 보존하고, 다음 기회를 찾습니다</h2><p>연도 선택 → 담당자 선택 → 해당 현장 확인. 전환 이후 영업은 Pipeline에서만 관리합니다.</p><div class="exp-kpis">'+[['선택기간 거래',yearRows.length],['전체연도 기한도래',globalDue.length],['선택기간 니즈확인',active.filter(r=>F.status(r)==='니즈확인').length],['선택기간 전환',made.length]].map(x=>'<div class="exp-kpi"><span>'+x[0]+'</span><b>'+x[1]+'</b></div>').join('')+'</div></section><nav class="exp-year-tabs" aria-label="확장관리 연도">'+yearTabs.map(y=>'<button data-v="'+h(y)+'" class="'+(String(year)===y?'on':'')+'" title="'+h(y==='이전'?(current-3)+'년 이전 거래':y+'년 거래')+'" onclick="G.expansionYear=this.dataset.v;G.expansionOwner=\'전체\';G.expansionPage=1;paintExpansion()">'+h(y)+'</button>').join('')+'</nav>'+ownerSummary(yearRows)+'<div class="exp-toolbar"><label>담당자 <select title="선택한 담당자의 확장관리 현장만 표시합니다." onchange="G.expansionOwner=this.value;G.expansionPage=1;paintExpansion()">'+owners.map(o=>'<option '+((G.expansionOwner||'전체')===o?'selected':'')+'>'+h(o)+'</option>').join('')+'</select></label><label>상태 <select title="확장관리 상태로 목록을 좁힙니다." onchange="G.expansionStatusFilter=this.value;G.expansionPage=1;paintExpansion()">'+['전체',...F.statuses].map(s=>'<option '+(s===statusFilter?'selected':'')+'>'+h(s)+'</option>').join('')+'</select></label><label>다음 접촉일 <select title="상태 조건을 유지한 채 접촉 기한으로 한 번 더 좁힙니다." onchange="G.expansionDueFilter=this.value;G.expansionPage=1;paintExpansion()"><option value="전체" '+(dueFilter==='전체'?'selected':'')+'>전체</option><option value="도래" '+(dueFilter==='도래'?'selected':'')+'>오늘·기한초과</option></select></label><label class="exp-search">검색 <span><input value="'+h(G.expansionQuery||'')+'" placeholder="현장·담당자·공종" onkeydown="if(event.key===\'Enter\'){G.expansionQuery=this.value;G.expansionPage=1;paintExpansion()}"><button onclick="G.expansionQuery=this.previousElementSibling.value;G.expansionPage=1;paintExpansion()">검색</button></span></label><span class="hint">선택 결과 '+rows.length+'건 · '+page+' / '+pageCount+'페이지</span></div><div class="exp-pool-list">'+(visible.map(card).join('')||'<p class="exp-empty">선택한 조건의 고객이 없습니다.</p>')+'</div>'+(rows.length>pageSize?'<div class="exp-more"><button '+(page<=1?'disabled':'')+' onclick="G.expansionPage='+(page-1)+';paintExpansion()">← 이전 20건</button><strong>'+page+' / '+pageCount+'</strong><button '+(page>=pageCount?'disabled':'')+' onclick="G.expansionPage='+(page+1)+';paintExpansion()">다음 20건 →</button></div>':'');
 }
 function card(r){
